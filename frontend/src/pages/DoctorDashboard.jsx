  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col md:flex-row pt-20 relative overflow-hidden">
      ...
      <div className="w-full md:w-64 bg-white/80 backdrop-blur-md md:border-r border-b md:border-b-0 border-slate-200 h-auto md:min-h-[calc(100vh-5rem)] flex-shrink-0 p-4 md:space-y-2 relative z-20">
        <h2 className="hidden md:block text-sm font-bold text-slate-400 uppercase tracking-wider mb-4 px-3 mt-4">Doctor Menu</h2>
        ...
        <div className="flex md:flex-col overflow-x-auto gap-2 pb-2 md:pb-0 snap-x scrollbar-hide" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
          <button onClick={() => setActiveTab('slots')} className={`flex-shrink-0 snap-start md:w-full flex items-center gap-3 px-4 py-3 rounded-xl font-semibold transition-colors ${activeTab === 'slots' ? 'bg-blue-50 text-blue-700' : 'text-slate-600 hover:bg-slate-50'}`}>
            <Clock size={20} /> <span className="whitespace-nowrap">Schedule</span>
          </button>
          ...
          <button onClick={() => setActiveTab('chat')} className={`flex-shrink-0 snap-start md:w-full flex items-center gap-3 px-4 py-3 rounded-xl font-semibold transition-colors ${activeTab === 'chat' ? 'bg-indigo-50 text-indigo-600' : 'text-slate-600 hover:bg-slate-50'}`}>
            <MessageCircle size={20} /> <span className="whitespace-nowrap">Patient Chat</span>
          </button>
          
          <button onClick={() => setActiveTab('profile')} className={`flex-shrink-0 snap-start md:w-full flex items-center gap-3 px-4 py-3 rounded-xl font-semibold transition-colors ${activeTab === 'profile' ? 'bg-slate-800 text-white' : 'text-slate-600 hover:bg-slate-50'}`}>
            <Settings size={20} /> <span className="whitespace-nowrap">Profile</span>
          </button>
        </div>
      </div>
